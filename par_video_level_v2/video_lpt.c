...
int main (int argc, char *argv[])
{
	...
	for (int i = 0; i < threads_num; i++) {
		if (i != threads_num - 1) {
			send_counts[i] = frames_nr / threads_num * frame_size;
			displs[i] = i * (frames_nr / threads_num * frame_size);
		} else {
			/* last thread gets what's remaining */
			send_counts[i] = (frames_nr / threads_num + frames_nr % threads_num) * frame_size;
			displs[i] = i * (frames_nr / threads_num * frame_size);
		}
	}
	...
	fread(frame_buffer, 1, frames_nr * frame_size, in);

	/* Add current frame to each thread */
	for (int i = 0; i < threads_num; i++)
		infos[i]->thread_frame = frame_buffer;
	...
	fwrite(frame_buffer, 1, frames_nr * frame_size, out);
