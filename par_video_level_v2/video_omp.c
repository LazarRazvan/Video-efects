...
int main(int argc, char **argv) {
	 ...
	 */
	if (argc != 6) {
		printf("Usage : ./filter width height filter num_threads\n");
		exit(-1);
	}
	...
	if (strcmp(filter, "sepia") == 0) {	
		#pragma omp parallel num_threads(num_threads)
		{
			...
			for (i = 0; i < nr_frames; i++)
				apply_sepia_on_frame(frame_buffer + i * frame_size, height, width);
		}			
	}
		...
		#pragma omp parallel num_threads(num_threads)
		{
			#pragma omp for private(i) schedule(dynamic)
			for (i = 0; i < nr_frames; i++)
				apply_negative_on_frame(frame_buffer + i * frame_size, height, width);
		}	
	}
		...
		#pragma omp parallel num_threads(num_threads)
		{
			#pragma omp for private(i) schedule(dynamic)
			for (i = 0; i < nr_frames; i++)
				apply_blur_on_frame(frame_buffer + i * frame_size, height, width);
		}	
	}
